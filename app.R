...
server <- function(input, output) {
  ...
  output$settlementPlot <- renderPlot({
    ...
    get_Expected_S_t_of_L_point <- function(lambda, alpha, J, c_p, c_d, k_p, k_d, t_endpoint, t){
      ...
      integrand <- function(x){ifelse(x==t_endpoint, 0, integrand_raw(x))}

      tryCatch({
        E_S_t <- 1.0/(1.0 - exp(-lambda * (t_endpoint - t))) * integrate(f = integrand, lower = t, upper = t_endpoint, stop.on.error=FALSE)$value
      },
      error = function(e) {
        print(paste0("lambda = ", lambda, "; alpha = ", alpha, "; t: ", t, "; endpoint: ", t_endpoint))}
      )
    ...
    #     eq <- get_Expected_S_t_of_L_point(alpha, Tmax, J, c_p, c_d, k_p, k_d, t) - get_s_star(lambda, y, c_p, c_d)
    #
    #     return(eq)
    #   }
    #
    #   unique_root <- uniroot(s_equation, c(0, Tmax-0.01),extendInt = "no")$root
    #   return(unique_root)
    # }
    ...
    get_t_star_star_solve <- function(y, alpha, Tmax, J, c_p, c_d, k_p, k_d){
      ...
      # if(get_Expected_S_t_of_L_point(lambda, alpha, J, c_p, c_d, k_p, k_d, Tmax, (Tmax-0.01)) < get_s_star(lambda, y, c_p, c_d)){
      #   return(Tmax)
      # }
      # else{
        s_equation <- function(t){
          eq <- get_Expected_S_t_of_L_point(lambda, alpha, J, c_p, c_d, k_p, k_d, Tmax, t) - get_s_star(lambda, y, c_p, c_d)
        ...
        out <- Tmax
        tryCatch({
          out <- uniroot(s_equation, c(0, Tmax-0.01),extendInt = "yes")$root
        },
        error = function(e) {
          print("No root found in the specified interval; t** = Tmax")
          out <- Tmax}
        )
    ...
    get_t_star_star_star_solve <- function(y, alpha, Tmax, J, c_p, c_d, k_p, k_d, lambda){
      ...
      get_s_star_star <- function(t){
        ...
        s_equation <- function(t){
          eq <- get_Expected_S_t_of_L_point(lambda, alpha, J, c_p, c_d, k_p, k_d, Tmax, t) - get_s_star_star(t)
